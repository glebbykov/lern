...
# Инспект — конкретные поля (читаемый вывод)
...
# Память: raw bytes → делить на 1048576 чтобы получить МБ
# Проще использовать docker stats --no-stream <name>

# Переменные окружения — по одной на строку
docker inspect <id> --format '{{range .Config.Env}}{{println .}}{{end}}'

# Сети контейнера — имя сети и IP
docker inspect <id> --format '{{range $net,$c := .NetworkSettings.Networks}}{{$net}}: {{$c.IPAddress}}{{println}}{{end}}'

# Маунты — тип, источник, куда, права
docker inspect <id> --format '{{range .Mounts}}{{.Type}} {{.Source}} -> {{.Destination}} ({{.Mode}}){{println}}{{end}}'

# Healthcheck — статус и последние проверки
docker inspect <id> --format 'status: {{.State.Health.Status}}'
docker inspect <id> --format '{{range .State.Health.Log}}exit={{.ExitCode}} out={{.Output}}{{end}}'
...
```bash
# Какие сети у client? (имя сети: IP)
docker inspect $(docker compose -f broken/compose-dns.yaml ps -q client) \
  --format '{{range $net,$c := .NetworkSettings.Networks}}{{$net}}: {{$c.IPAddress}}{{println}}{{end}}'
# 06-debug-troubleshooting_frontend: 172.x.x.x

# Какие сети у api?
docker inspect $(docker compose -f broken/compose-dns.yaml ps -q api) \
  --format '{{range $net,$c := .NetworkSettings.Networks}}{{$net}}: {{$c.IPAddress}}{{println}}{{end}}'
# 06-debug-troubleshooting_backend: 172.x.x.x  ← другая сеть!
...
> **Как работает:** dd пишет 30 МБ в `/dev/shm` — это tmpfs (RAM), запись идёт напрямую в память.
> `memswap_limit` равен `mem_limit` — swap отключён. Ядро видит превышение лимита cgroup и отправляет SIGKILL.
...
# Убит ли именно OOM killer'ом?
docker inspect dbg-oom --format '{{.State.OOMKilled}}'
# true  ← если false при ExitCode=137, значит docker kill, не OOM

# Сколько памяти было разрешено (в байтах → делить на 1048576 = МБ)
docker inspect dbg-oom --format '{{.HostConfig.Memory}}'
# 8388608 → 8388608 / 1048576 = 8 МБ

# Проще: human-readable через stats (пока контейнер ещё жив)
docker stats --no-stream dbg-oom
# CONTAINER  CPU %  MEM USAGE / LIMIT  MEM %
# dbg-oom    ...    7.9MiB / 8MiB      98%  ← почти достиг лимита

# Логи до момента убийства
docker logs dbg-oom
# writing to /dev/shm...
# 8+0 records in  ← обрывается здесь
```

**Root cause:** лимит памяти 8 MiB, dd пытается выделить 30 MiB через tmpfs — OOM killer отправил SIGKILL.

**Fix:** увеличить `mem_limit` до реального потребления. Сначала измерить без лимита:
...
docker inspect dbg-unhealthy --format '{{.State.Health.Status}}'
# unhealthy

# Последние результаты проверок — exit code и вывод
...
docker exec dbg-unhealthy wget -qO- http://localhost/
# <!DOCTYPE html>...  ← / работает, /healthz — нет
```
...
# Что примонтировано (тип, источник, куда, права)
docker inspect dbg-readonly \
  --format '{{range .Mounts}}{{.Type}} {{.Source}} -> {{.Destination}} ({{.Mode}}){{println}}{{end}}'
# (пусто — нет ни одного tmpfs для записи)

# Есть ли tmpfs-маунты (директории для записи при read_only: true)
docker inspect dbg-readonly \
  --format '{{range $path,$opts := .HostConfig.Tmpfs}}tmpfs: {{$path}} {{$opts}}{{println}}{{end}}'
# (пусто — не настроено, поэтому запись невозможна)
```
...
```bash
# Переменные окружения внутри контейнера — по одной на строку
docker inspect dbg-missing-env --format '{{range .Config.Env}}{{println .}}{{end}}'
# PATH=/usr/local/sbin:...
# HOSTNAME=...
# DATABASE_URL отсутствует в списке

# Альтернатива — если контейнер ещё жив (при restart: on-failure можно успеть)
docker exec dbg-missing-env env | grep DATABASE || echo 'не найдена'
```
...
# Маунты: тип, источник на хосте, куда в контейнере
docker inspect dbg-vol-perm \
  --format '{{range .Mounts}}{{.Type}} {{.Name}} -> {{.Destination}}{{println}}{{end}}'
# volume  06-debug-troubleshooting_vol-perm-data -> /data

# Права на директорию в volume (смотрим снаружи, от root)
docker run --rm \
  -v 06-debug-troubleshooting_vol-perm-data:/data \
  alpine:3.20 ls -lan /data
# drwxr-x--- 2 0 0  ← uid=0 (root), gid=0, права 750 — UID 1000 не пройдёт
```
...
# Опубликованные порты (хост:контейнер)
docker inspect dbg-listen \
  --format '{{range $port,$binds := .NetworkSettings.Ports}}{{$port}} -> {{$binds}}{{println}}{{end}}'
# (пусто) — порты не опубликованы, хотя EXPOSE 80 есть в образе
```
