...
- Никогда осознанно не смотрел `ps`, `top`, `/proc`
- Не понимаешь разницу между `kill` и `kill -9`
- Не знаешь что такое «файловый дескриптор»
- Хочешь сначала «потрогать», а потом копаться в теории

Если уже понимаешь половину — пропускай сразу к Лабе 01.
...
## Задание 1. Смотрим процессы глазами
...
## Задание 2. /proc — живая документация процесса

У каждого процесса в `/proc/PID/` есть «паспорт» — псевдофайлы, которые ядро генерирует на лету.
...
## Задание 3. Файловые дескрипторы: что там «открыто» у процесса
...
## Задание 4. Сигналы: как «попросить» процесс что-то сделать
...
> **Правило хорошего тона:** `kill -9` — это крайняя мера. Нормальное завершение —
> через `SIGTERM`: процесс сам закроет сокеты, сбросит буферы, удалит lock-файлы.
> Только если процесс не реагирует на `SIGTERM` — тогда `-9`.

### Практика

```bash
# Скрипт, который перехватывает SIGTERM и "прибирается"
...
PID=$!
sleep 2
ls -la /tmp/mylock   # файл есть

# Пошли SIGTERM (вежливо)
kill $PID
sleep 1
ls -la /tmp/mylock   # файла НЕТ — скрипт успел убрать
wait $PID 2>/dev/null

# Теперь попробуем "жестко" — SIGKILL
/tmp/graceful.sh &
PID=$!
sleep 2
...
kill -9 $PID
sleep 1
ls -la /tmp/mylock   # файл всё ещё ЕСТЬ — скрипт не успел убрать
rm -f /tmp/mylock /tmp/graceful.sh
wait $PID 2>/dev/null
```

> После `kill -9` bash напечатает `-bash: line N: <PID> Killed /tmp/graceful.sh` —
> это не ошибка, а отчёт встроенной системы job control о том, что фоновый процесс умер.
...
## Задание 5. Свой systemd-сервис: запусти и перезапусти
...
## Задание 6. Зомби: процесс мёртв, но запись жива

**Зомби-процесс** — это процесс, который **завершился**, но его родитель ещё не
вызвал `wait()`, чтобы забрать «свидетельство о смерти» (exit code).

Ядро хранит небольшую запись о таком процессе, пока родитель не прочитает её.
Это нужно, чтобы родитель знал: «вот мой ребёнок, вот с каким кодом он умер».

### Практика

```bash
# Python-скрипт, который плодит зомби
...
for i in range(3):
    pid = os.fork()
    if pid == 0:
        os._exit(i)   # ребёнок сразу умирает

# Родитель НЕ вызывает wait() — дети становятся зомби
...
# Найди зомби (состояние Z)
echo "--- Зомби-процессы ---"
ps --ppid $PARENT -o pid,state,comm

# Попробуй убить зомби (не получится — они уже мертвы)
for z in $(ps --ppid $PARENT -o pid= --no-headers); do
  kill -9 $z
done
echo "--- После kill -9: зомби всё ещё здесь ---"
ps --ppid $PARENT -o pid,state,comm

# А теперь убей РОДИТЕЛЯ
kill $PARENT
sleep 2
echo "--- После смерти родителя: зомби ушли ---"
...
1. Почему `kill -9` не убил зомби?
   (Подсказка: чтобы убить процесс — он должен быть жив. Зомби уже мёртв.)
2. Почему после смерти родителя зомби исчезают? Кто их «убирает»?
   (Подсказка: когда родитель умирает, детей усыновляет PID 1 — `init`/`systemd`.
   И этот init регулярно вызывает `wait()` на своих детях.)
3. Где реально проблема зомби: если их накопится **тысячи** — что закончится в системе?
...
Три способа запустить процесс, чтобы он «не завис» на терминале.

| Способ | Переживёт закрытие терминала? | Как |
|--------|-------------------------------|-----|
| `cmd &` | ❌ Нет (получит SIGHUP) | Запуск в фоне |
| `nohup cmd &` | ✅ Да | Ставит SIG_IGN на SIGHUP + лог в nohup.out |
| `cmd &` + `disown` | ✅ Да (в bash) | Удаляет из job table — SIGHUP не шлётся |
...
1. Что такое SIGHUP и когда bash рассылает его фоновым процессам?
2. Почему `nohup` перенаправляет stdout в файл `nohup.out`?
   (Подсказка: если терминал закрыт, запись в него даст SIGPIPE.)
3. Чем `disown` отличается от `nohup` — что именно он «отключает»?
...
## Задание 8. Межпроцессное взаимодействие (IPC): pipes и FIFO

Процессы не живут в вакууме — им нужно обмениваться данными. Самые
простые механизмы: **каналы** (pipes) и **именованные каналы** (FIFO).

### Практика: anonymous pipe

Каждый раз, когда ты пишешь `|`, shell создаёт anonymous pipe.

```bash
# Под капотом: ps открывает pipe, fork()'ает grep, соединяет fd 1 ps → fd 0 grep
ps aux | grep bash | wc -l
# Ожидаемо: небольшое число, зависит от сессии

# Посмотрим pipe своими глазами. Запустим pipeline в фоне.
sleep 100 | cat &
# $! — PID последней команды pipeline (cat). PID первой (sleep) нужно искать:
PID_CAT=$!
PID_SLEEP=$(ps -o pid= --ppid $$ -C sleep | head -1 | tr -d ' ')

echo "sleep=$PID_SLEEP  cat=$PID_CAT"
ls -la /proc/$PID_SLEEP/fd/   # fd 1 → pipe:[N]  ← писатель
ls -la /proc/$PID_CAT/fd/     # fd 0 → pipe:[N]  ← читатель (тот же номер)

kill $PID_SLEEP $PID_CAT 2>/dev/null
wait 2>/dev/null
```

> **Как это понять:** pipe — это два связанных fd. Ядро создаёт буфер,
> один конец отдаёт писателю (fd 1 у `sleep`), другой — читателю (fd 0
> у `cat`). Оба fd видят один и тот же inode `pipe:[N]`.
...
## Задание 9. Отладка процессов: strace и lsof — что процесс делает прямо сейчас
